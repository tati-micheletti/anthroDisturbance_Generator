...
testthat::test_that("Generator runs on testing AOI forestry data", {
  skip_on_cran()
  req_pkgs <- c("SpaDES.core", "SpaDES.tools", "reproducible", "terra",
                "sf", "data.table", "fasterize", "digest")
  for (pkg in req_pkgs) testthat::skip_if_not_installed(pkg)
  
  fixtures <- load_testing_aoi_fixtures(resolution = 60)
  testthat::skip_if(terra::nrow(fixtures$cutblocks) == 0,
                    "Testing AOI cutblocks fixture is empty")
  testthat::skip_if(terra::nrow(fixtures$potentialCutblocks) == 0,
                    "Testing AOI potential cutblocks fixture is empty")
  
  # Attach packages the module expects on the search path
  attach_pkg_safely <- function(pkg) {
    if (paste0("package:", pkg) %in% search()) return(invisible(TRUE))
    ok <- try(suppressPackageStartupMessages(
  ...
  paramsList <- list(
    anthroDisturbance_Generator = list(
      .seed = list(
        anthroDisturbance_Generator = list(
          calculatingSize = 401L,
          calculatingRate = 402L,
          generatingDisturbances = 403L,
          updatingDisturbanceList = 404L
        )
      ),
      runInterval = 1,
      saveInitialDisturbances = FALSE,
  ...
  sim <- suppressWarnings(SpaDES.core::simInit(
    ...
    objects = list(
      ...
      DEM                   = { x <- fixtures$raster; terra::values(x) <- 300; x },
      rstCurrentBurn        = { x <- fixtures$raster; terra::values(x) <- 0;   x },
      DisturbanceRate       = data.table::data.table(),
      disturbanceList       = list(
        forestry = list(
  ...
  final_area <- sum(terra::expanse(final_cutblocks, unit = "m", transform = FALSE))
